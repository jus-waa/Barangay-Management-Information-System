...
if (isset($_POST['confirm'])) {
    try{
        ...
        $query = "INSERT INTO `print_history`(`first_name`, `middle_name`, `last_name`,  `suffix`, `age`, `document_type`, `barangay_name` ,`print_date`, `control_number`,  `issued_by`, `purpose`) VALUES (:first_name, :middle_name, :last_name, :suffix, :age, :document_type, :barangay_name, :print_date, :control_number, :issued_by, :purpose)";
        ...
        if ($stmt) {
            //header("Location: print/certindigency_print.php?id=" . $lastInsertId);
            echo "<script>
                window.open('print/certindigency_print.php?id=$lastInsertId', '_blank');
...
if (isset($_POST['confirmPrintSignature'])) {
    try{
        ...
        if ($stmt) {
            //header("Location: print/certindigency_print.php?id=" . $lastInsertId);
            echo "<script>
                window.open('print/certindigency_print_signatured.php?id=$lastInsertId', '_blank');
                window.open('../generatedocuments.php', '_self');
            </script>";
            exit;
        }
            ...
            <form method="post" id="personal_info">
                ...
                <div class="grid grid-cols-2 gap-10 std:gap-20">
                    ...
                    <div>
                        ...
                        <div class="rounded-lg p-2 mb-8">
                            <div>
                                <h2 class="text-lg font-bold mb-4"> Control Number & Date of Issuance</h2>
                                <div class="border-2 grid grid-cols-1 gap-4 p-6 rounded-md hover:border-sg transition duration-700 text-sm std:text-base <?= $isEnabled ? '' : 'hover:animate-shake' ?>">
                                    <?php 
                                    // fetch data
                                    ...
                                    foreach ($print as $rows) {
                                        ...
                                        if($rows['document_type'] === 'Certificate of Indigency') {
                                            $certIndigency++;
                                        }
                                    }
                                    $controlNumber = $certIndigency;
                                    $formattedControlNumber = sprintf("%04d", $controlNumber);
                                    ?>
                                    <div>
                                        <input id="control-number" name="control_number" type="text" autocomplete="off" class="block bg-transparent w-full border-2 border-gray-200 p-2 peer rounded-md focus:outline-none focus:border-sg pointer-events-none" value="CI-<?= $formattedControlNumber ?>" placeholder=" "/> 
                ...
                <div class="flex justify-between items-center">
                    ...
                    <a href="print/certindigency_print.php?id=<?= $row['id']?>">
                                ...
                                <div class="border-2 border-c rounded-lg mx-4">
                                                ...
                                                <tr class="hover:bg-gray-100 text-sm std:text-base">
                                                    ...
                                                    <td class="border-y-2 border-c">
                                                        <div class="flex justify-center items-center h-10 std:h-14 grow">
                                                            <a href="certindigency.php?id=<?= $row['id']?>">
                                    ...
                                    <div class=" h-12 rounded-b-sm bg-c">
                                        <?php
                                            ...
                                            if ($page > 1) {
                                                echo "<a href='certindigency.php?page=1&search=$search' class='px-4 py-2 text-sm  text-white bg-sg rounded-l-lg hover:opacity-80'>&laquo; First</a>";
                                                echo "<a href='certindigency.php?page=" . ($page - 1) . "&search=$search' class='px-4 py-2 text-sm  text-white bg-sg hover:opacity-80'>&lt; Previous</a>"; // Previous page link
                                            } else {
                                            ...
                                            for ($i = max(1, $page - 5); $i <= min($total_pages, $page + 5); $i++) {
                                                if ($i == $page) {
                                                    ...
                                                    echo "<a href='certindigency.php?page=" . $i . "&search=$search' class='px-4 py-2 text-sm  text-white bg-sg hover:opacity-80'>" . $i . "</a>";
                                                }
                                            }
                                            // Display next and last
                                            if ($page < $total_pages) {
                                               echo "<a href='certindigency.php?page=" . ($page + 1) . "&search=$search' class='px-4 py-2 text-sm  text-white bg-sg hover:opacity-80'>Next &gt;</a>"; // Next page link
                                               echo "<a href='certindigency.php?page=$total_pages&search=$search' class='px-4 py-2 text-sm  text-white bg-sg rounded-r-lg hover:opacity-80'>Last &raquo;</a>"; // Last page link
                                            } else {
