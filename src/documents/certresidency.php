...
if (isset($_POST['confirm'])) {
    try{
        ...
        $query = "INSERT INTO `print_history`(`first_name`, `middle_name`, `last_name`,  `suffix`, `age`, `document_type`, `barangay_name` ,`print_date`, `birth_date` , `control_number`,  `issued_by`, `purpose`) VALUES (:first_name, :middle_name, :last_name, :suffix, :age, :document_type, :barangay_name, :print_date, :birth_date, :control_number, :issued_by, :purpose)";
        ...
        if ($stmt) {
            //header("Location: print/certresidency_print.php?id=" . $lastInsertId);
            echo "<script>
                window.open('print/certresidency_print.php?id=$lastInsertId', '_blank');
...
if (isset($_POST['confirmPrintSignature'])) {
    try{
        ...
        if ($stmt) {
            //header("Location: print/certresidency_print.php?id=" . $lastInsertId);
            echo "<script>
                window.open('print/certresidency_print_signatured.php?id=$lastInsertId', '_blank');
                window.open('../generatedocuments.php', '_self');
            </script>";
            exit;
        }
                            ...
                            <div>
                                ...
                                <div class="border-2 grid grid-cols-1 gap-4 p-6 text-sm std:text-base rounded-md hover:border-sg transition duration-700 <?= $isEnabled ? '' : 'hover:animate-shake' ?>">
                                    ...
                                    foreach ($print as $rows) {
                                        ...
                                        if($rows['document_type'] === 'Certificate of Residency') {
                                            $certresidency++;
                                        }
                                    }
                                    $controlNumber = $certresidency;
                                    $formattedControlNumber = sprintf("%04d", $controlNumber);
                                    ?>
                                    <div>
                                        <input id="control-number" name="control_number" type="text" autocomplete="off" class="block bg-transparent w-full border-2 border-gray-200 p-2 peer rounded-md focus:outline-none focus:border-sg pointer-events-none" value="CR-<?= $formattedControlNumber ?>" placeholder=" "/> 
                                    ...
                                    <div>
                                        <input id="date-of-birth" name="birth_date" type="date" autocomplete="off" class="block bg-transparent w-full border-2 border-gray-200 p-2 peer rounded-md focus:outline-none focus:border-sg pointer-events-none" value="<?php echo $recordLoaded ? $row['birth_date'] : $defaultValues['birth_date']?>" placeholder=" "/> 
                                        <label class="absolute text-gray-500 pointer-events-none text-sm duration-300  transform -translate-y-14 -translate-x-1 pr-2 scale-75 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-8 peer-placeholder-shown:translate-x-2 peer-focus:scale-75 peer-focus:-translate-x-1 peer-focus:-translate-y-14 z-10 bg-white pl-1 text-left rounded-2xl">Date of Birth</label>
                                    ...
                                    <div>
                                        ...
                                        <label class="absolute text-gray-500 pointer-events-none text-sm duration-300  transform -translate-y-14 -translate-x-1 pr-2 scale-75 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-8 peer-placeholder-shown:translate-x-2 peer-focus:scale-75 peer-focus:-translate-x-1 peer-focus:-translate-y-14 z-10 bg-white pl-1 text-left rounded-2xl">Issued On</label>
                                ...
                                <div class="border-2 border-c rounded-lg mx-4">
                                    ...
                                    <div class=" h-12 rounded-b-sm bg-c">
                                        <?php
                                            ...
                                            if ($page > 1) {
                                                echo "<a href='certresidency.php?page=1&search=$search' class='px-4 py-2 text-sm  text-white bg-sg rounded-l-lg hover:opacity-80'>&laquo; First</a>";
                                                echo "<a href='certresidency.php?page=" . ($page - 1) . "&search=$search' class='px-4 py-2 text-sm  text-white bg-sg hover:opacity-80'>&lt; Previous</a>"; // Previous page link
                                            } else {
                                            ...
                                            for ($i = max(1, $page - 5); $i <= min($total_pages, $page + 5); $i++) {
                                                if ($i == $page) {
                                                    ...
                                                    echo "<a href='certresidency.php?page=" . $i . "&search=$search' class='px-4 py-2 text-sm  text-white bg-sg hover:opacity-80'>" . $i . "</a>";
                                                }
                                            }
                                            // Display next and last
                                            if ($page < $total_pages) {
                                               echo "<a href='certresidency.php?page=" . ($page + 1) . "&search=$search' class='px-4 py-2 text-sm  text-white bg-sg hover:opacity-80'>Next &gt;</a>"; // Next page link
                                               echo "<a href='certresidency.php?page=$total_pages&search=$search' class='px-4 py-2 text-sm  text-white bg-sg rounded-r-lg hover:opacity-80'>Last &raquo;</a>"; // Last page link
                                            } else {
