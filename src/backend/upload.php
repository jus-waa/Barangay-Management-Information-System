...
if($_FILES["file"]["size"] > 0) {
    ...
    while(($data = fgetcsv($file, 10000, ",")) !== FALSE) {
        ...
        try {
            ...
            $stmt->bindParam(':height', $data[22], PDO::PARAM_INT);
            $stmt->bindParam(':weight', $data[23], PDO::PARAM_INT);
            $stmt->bindParam(':eye_color', $data[24], PDO::PARAM_STR);
            $stmt->bindParam(':blood_type', $data[25], PDO::PARAM_STR);
            $stmt->bindParam(':religion', $data[26], PDO::PARAM_STR);
            for($i=0;$i<=26;$i++) {
                if($data[$i] == null) {
                    $data[$i] = "";
                }
