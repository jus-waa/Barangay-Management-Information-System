...
if($_FILES["file"]["size"] > 0) {
    ...
    while(($data = fgetcsv($file, 10000, ",")) !== FALSE) {
        ...
        try {
            ...
            $stmt->bindParam(':father_name', $data[9], PDO::PARAM_STR);
            $stmt->bindParam(':mother_maiden_name', $data[10], PDO::PARAM_STR);
            $stmt->bindParam(':contact_num', $data[11], PDO::PARAM_STR);
            $stmt->bindParam(':email_address', $data[12], PDO::PARAM_STR);
            $stmt->bindParam(':house_num', $data[13], PDO::PARAM_STR);
            $stmt->bindParam(':street_name', $data[14], PDO::PARAM_STR);
            $stmt->bindParam(':barangay_name', $data[15], PDO::PARAM_STR);
            $stmt->bindParam(':municipality_city', $data[16], PDO::PARAM_STR);
            $stmt->bindParam(':province', $data[17], PDO::PARAM_STR);
            $stmt->bindParam(':zip_code', $data[18], PDO::PARAM_STR);
            $stmt->bindParam(':civil_status', $data[19], PDO::PARAM_STR);
            $stmt->bindParam(':citizenship', $data[20], PDO::PARAM_STR);
            $stmt->bindParam(':ethnicity', $data[21], PDO::PARAM_STR);
            $stmt->bindParam(':occupation', $data[22], PDO::PARAM_STR);
            $stmt->bindParam(':residency_type', $data[23], PDO::PARAM_STR);
            $stmt->bindParam(':status', $data[24], PDO::PARAM_STR);
            $stmt->bindParam(':height', $data[25], PDO::PARAM_INT);
            $stmt->bindParam(':weight', $data[26], PDO::PARAM_INT);
            $stmt->bindParam(':eye_color', $data[27], PDO::PARAM_STR);
            $stmt->bindParam(':blood_type', $data[28], PDO::PARAM_STR);
            $stmt->bindParam(':religion', $data[29], PDO::PARAM_STR);
            for($i=0;$i<=29;$i++) {
                if($data[$i] == null) {
                    $data[$i] = "";
                }
