...
try {
    ...
    if (empty($search)) {
        ...
        $stmt3 = $dbh->prepare("SELECT * FROM `print_history` WHERE DATE(print_date) >= :startOfWeek LIMIT :limit OFFSET :offset");
    ...
    if (empty($prevSearch)) {
        ...
        $stmt4 = $dbh->prepare("SELECT * FROM `print_history`
                                ...
                                ) AND DATE(print_date) < :startOfWeek ORDER BY CASE
                                        WHEN first_name LIKE :prevSearch THEN 1
